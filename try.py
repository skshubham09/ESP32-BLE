...
LOG_API_URL = "https://cms-backend-five.vercel.app/api/log/createLogs"  # API to log connection status
...
def log_device_status(is_connected):
    """
    Send a POST request to log the connection status (connected/disconnected).
    """
    log_data = {
        "id": DEVICE_ID,
        "message": "Device connected" if is_connected else "Device disconnected"
    }
    
    try:
        response = requests.post(LOG_API_URL, json=log_data)
        if response.status_code == 200:
            print(f"Connection status logged successfully: {log_data['message']}")
        else:
            print(f"Failed to log connection status. Status code: {response.status_code}")
    except Exception as e:
        print(f"Error logging connection status: {e}")
    ...
    while True:
        ...
        if last_device_id_timestamp and (current_time - last_device_id_timestamp <= 5):
            if not is_connected:
                ...
                log_device_status(is_connected)  # Log connected status
        else:
            if is_connected:
                print("Device disconnected.")
                is_connected = False  # Update status when device disconnects
                log_device_status(is_connected)  # Log disconnected status
